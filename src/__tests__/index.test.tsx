...
describe('SandboxAccessFormLegacy', () => {
  ...
  describe('ouath ccg apis', () => {
    it('adds required fields and links with CCG API', async () => {
      ...
      expect(screen.getByText('Learn how to generate a public key.')).toHaveAttribute('href', defaultUrls.ccgPublicKeyUrl);
    });
  });
  ...
  describe('submit button', () => {
    ...
    it('triggers validation when clicked', async () => {
      ...
      setTimeout(() => {
        expect(screen.findByText('Enter a valid email address.')).toBeInTheDocument();
      }, 0);
    });
    ...
    it('validates oauth fields when clicked', async () => {
      ...
      setTimeout(() => {
        ...
        expect(screen.getByText('Enter an http or https URI.')).toBeInTheDocument();
      }, 0);
    });
    ...
    it('validates internal api fields when clicked and does not ask for VA email if a VA email exists in the dev info email field', async () => {
      ...
      .then(() => {
        ...
        expect(screen.getByText('Enter your program name.')).toBeInTheDocument();
        expect(screen.getByText('Enter a valid VA-issued email address.')).toBeInTheDocument();
